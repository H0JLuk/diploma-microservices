...
export class PrismaClientExceptionFilter extends BaseExceptionFilter {
   ...
   * @returns
   */
  catch(exception: Prisma.PrismaClientKnownRequestError | Prisma.NotFoundError | unknown, host: ArgumentsHost) {
  ...
  private exceptionShortMessage(message: string): string {
    const shortMessage = message.slice(Math.max(0, message.indexOf('→')));
